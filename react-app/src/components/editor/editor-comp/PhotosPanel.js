...
const PhotosPanel = ({photosOfPanel, getPhotoName, deletePhoto, choosenLanguage}) => {
  ...
  return (
    ...
    <div style={{ width: '100%', height: '30px', backgroundColor: 'black', borderTopLeftRadius: '10px', borderTopRightRadius: '10px', color: 'white' }}>
        <div style={{ display: 'flex', color: 'white' }}>
            <Typography style={{ marginTop: '3px', marginLeft: '5px' }}>{choosenLanguage === 'EN' ? (`Photos`) : (`Zdjęcia`)}</Typography>
        </div>
    </div>
        ...
        {photosOfPanel.length > 0 ? (
            photosOfPanel.filter(photoUrl => !photoUrl.includes('data.json')).map((photoUrl, index) => (
                <div key={photoUrl} style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
                    ...
                    <div onClick={() => deletePhoto(photoUrl)} style={{ marginRight: '10px', marginLeft: 'auto', cursor: 'pointer' }}>{choosenLanguage === 'EN' ? (`Delete`) : (`Usuń`)}</div>
                </div>
            ))
        ) : (
